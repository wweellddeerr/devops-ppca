...
Desde então, a questão de como o desenvolvimento de \textit{software} deve ser
...
que adotaram \textit{DevOps} e que eventualmente poderiam ser usados por
...
Havia a premissa de que o \textit{software} deve ir pra produção a cada
...
times. São também enumerados os princípios e práticas utilizados na companhia
...
\subsection{Desafios na Adoção de \textit{DevOps}}
...
desafios relacionados à adoção de \textit{DevOps}. O primeiro desafio
apresentado, é a existência de \emph{comunicação insuficiente} entre os times
de desenvolvimento e operações. Em segundo lugar, como \textit{DevOps} requer
...
para execução de testes. Por fim, o quarto desafio apontado é que \textit{DevOps}
ainda não é completamente compreendido mas continua evoluindo rapidamente.

Já o trabalho de Smeds et al. \cite{devops_a_definition} as percepções de
praticantes form utilizadas para gerar um conjunto de onze possíveis
